...
class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneAction = UIContextualAction(style: .normal, title: doneTitle) { _, _, isDone in
            StorageManager.shared.changeComplitionState(for: task, isComplete: indexPath.section == 0)
            
            let rowDestination = indexPath.section == 0 ? self.completedTasks.index(of: task)
                                                        : self.currentTasks.index(of: task)
            
            let indexPathMoveTo = IndexPath(row: rowDestination ?? 0,
                                            section: indexPath.section == 0 ? 1 : 0
            )
            
            tableView.moveRow(at: indexPath, to: indexPathMoveTo)
